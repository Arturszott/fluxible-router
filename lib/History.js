...
History.prototype = {
    ...
    getUrl: function () {
        ...
        if (baseURI) {
            strippedBaseURI = this.getBasePrefix(document);
    ...
    getBasePrefix: function(document) {
        if (document.baseURI) {
    ...
    pushState: function (state, title, url) {
        var win = this.win;
        var base = this.getBasePrefix(win.document);
        if (this._hasPushState) {
            title = isUndefined(title) ? win.document.title : title;
            url = isUndefined(url) ? win.location.href : (path.join(base, url));
            ...
            win.location.href = path.join(base, url);
        }
    },
